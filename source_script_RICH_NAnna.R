...
args_all = list(park = park, from = from, to = to, QAQC = QAQC, locType = "all")
args_4yr = list(park = park, from = from_4yr, to = to, QAQC = QAQC, locType = "all")
args_vs = list(park = park, from = from, to = to, QAQC = QAQC, locType = "all")
...
new_path = paste0(path, park, "_NAnna/")
...
cycle_labs <- c("1" = "Cycle 1: 2026")

# Read in tree species groups for regen and tree pie charts
trspp_grps <- read.csv("NPS_tree_species_groups.csv")

# Source files
# source('./scripts/forest_summary_code_RICH_NAnna.R')
# source('./scripts/regen_debt_metrics_RICH_NAnna.R')
# source('./scripts/tree_regen_stem_density_by_species_RICH_NAnna.R')
