...
# 3. Update cycle_labs in source_script_MIDN.R
...
forestMIDN::importData(instance = "server", server = server_add)
...
} else {source('./scripts/forest_summary_code_MIDN.R')} # main figures and tables
```
```{r include = F, cache = F}
...
check_inv <- sum(rowSums(inv_cycle_tab[,3:ncol(inv_cycle_tab)], na.rm = T))
...
} else {cat(paste0("No invasive plants have been detected in ", park_long))}
```
```{r}
...
cat(paste0("<p><b>Figure 2.</b> Deer browse impacts by cycle in ", park_long, " (", park, "). ",
    ...
    ifelse(park %in% c("VAFO", "HOFU", "GETT", "APCO", "BOWA", "PETE", "FRSP", "RICH"), 
           "Note that Cycle 1 only includes 3 out of 4 panels of plots. ",
      ifelse(park %in% c("GEWA", "THST"), 
             "Note that CYcle 1 only includes 2 out of 4 panels of plots. ", 
             "")),
    ifelse(length(cycle_labs) == 5, "<b>Note that Cycle 5 is only partially complete.</b>", ""), 
    ifelse(park == "COLO", "<b>Note that Cycle 4 only partially complete.</b>", ""),
    sep = ""))
...
```{r}
fig6_eval <- if(nrow(guilds) > 0){TRUE} else {FALSE}
```

### {.page-break-before}
```{r results = 'asis', warning = F, message = F, eval = fig6_eval}
includeHTML("html_header.html")
```

```{r out.width = "98%", out.height = "50%", eval = fig6_eval}
...
```{r results = 'asis', eval = !fig6_eval}
cat("<p>Invasive plant detections not sufficient to model trends in percent cover by guild in ", 
    park_long, ". This is a good and rare occurence. </p>")
```
...
```{r echo = F, eval = tab2_eval, warning = F, message = F, results = 'asis'}
if(check_inv > 0){includeHTML("table2.html")
} else {cat(paste0("<p>No invasives plants were detected in ", park_long, ".</p>"))}
```
