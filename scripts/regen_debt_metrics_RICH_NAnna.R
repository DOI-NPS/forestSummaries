...
num_plots <- length(NAnna_plots)
plotevs <- joinLocEvent(park = park, from = from_4yr, to = to, locType = 'all') |> #filter(IsStuntedWoodland == FALSE) |> 
  select(Plot_Name, SampleYear) |> filter(Plot_Name %in% NAnna_plots) |>  # remove if plot sampled again
  change_cycle() 
# Deer Browse Index
dbi <- joinStandData(park = park, from = from_4yr, to = to, locType = 'all') |> #filter(IsStuntedWoodland == FALSE) |> 
  select(Plot_Name, dbi = Deer_Browse_Index) |> filter(Plot_Name %in% NAnna_plots)  |> change_cycle() 
...
dbi_all <- joinStandData(park = park, from = from, to = to, locType = 'all') |> 
  select(Plot_Name, cycle, dbi = Deer_Browse_Index) |> 
...
figpath <- paste0(path, park, '_NAnna/figures/')
...
reg <- joinRegenData(park = park, from = from_4yr, to = to, units = 'sq.m', locType = 'all') |> 
  filter(Plot_Name %in% NAnna_plots)  |> change_cycle() 
...
trees <- joinTreeData(park = park, from = from_4yr, to = to, status = 'live', locType = 'all') |> 
  filter(Plot_Name %in% NAnna_plots) |> change_cycle() |>   
  mutate(genus = word(ScientificName, 1),
         species = ifelse(is.na(word(ScientificName, 2)), "SPP", word(ScientificName, 2)),
         sppcode1 = toupper(paste0(substr(genus, 1, 3), substr(species, 1, 3))),
...
tree_dist <- sumTreeDBHDist(park = park, from = from_4yr, to = to, status = 'live', locType = 'all') |>
  filter(Plot_Name %in% NAnna_plots)  |> change_cycle() 
