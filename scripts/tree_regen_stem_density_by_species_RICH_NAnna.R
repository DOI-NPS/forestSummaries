...
wd_spp <- sumSpeciesList(park = "RICH", from = from, to = to, locType = 'all') |> 
  filter(Plot_Name %in% NAnna_plots) |> 
  filter(ScientificName %in% trspp_grps$Species) |> change_cycle() 

wd_spp2 <- left_join(wd_spp, trspp_grps |> select(Species, spp_grp), 
                     by = c("ScientificName" = "Species"))
wd_spp2$spp_grp[wd_spp2$ScientificName %in% c("Ulmus americana", "Ulmus rubra")] <- "Other Native"

wd_spp2 <- wd_spp2 %>% mutate(spp_grp = case_when(spp_grp == "Other Native" ~ "Other native canopy spp.",
...
tr_dens_plot <- 
  ggplot(tr_sum, aes(y = avg_dens_ha, x = "Tree Density", fill = spp_grp)) + # fct_reorder(spp_grp, avg_dens_ha))) + 
    geom_bar(position = 'stack', stat = 'identity', color = 'dimgrey') + 
    labs(y = "tree stems/ha") +
...
tr_BA_plot <- 
  ggplot(tr_sum, aes(y = avg_BA_m2ha, x = "Tree BA", fill = spp_grp)) + #fct_reorder(spp_grp, avg_BA_m2ha))) + 
  geom_bar(position = 'stack', stat = 'identity', color = 'dimgrey') + 
  labs(y = "tree basal area (sq.m/ha)") +
  scale_fill_manual(values = cols,  name = NULL) +
  theme_FVM() + theme(axis.title.x = element_blank(),
                      legend.title = element_blank(), 
                      legend.position = 'none')
...
seed_dens_plot <- 
  ggplot(regen_sum, aes(y = avg_seed_dens, x = "Seedling Density", fill = spp_grp)) + #fct_reorder(spp_grp, avg_seed_dens))) + 
  geom_bar(position = 'stack', stat = 'identity', color = 'dimgrey') + 
  labs(y = "seedling stems/sq.m") +
  scale_fill_manual(values = cols,  name = NULL) +
  theme_FVM() + theme(axis.title.x = element_blank(),
                      legend.title = element_blank(), 
                      legend.position = 'none')
...
sap_dens_plot <- 
  ggplot(regen_sum, aes(y = avg_sap_dens, x = "Sapling Density", fill = spp_grp)) + #fct_reorder(spp_grp, avg_sap_dens))) + 
  geom_bar(position = 'stack', stat = 'identity', color = 'dimgrey') + 
  labs(y = "sapling stems/sq.m") +
  scale_fill_manual(values = cols,  name = NULL) +
  theme_FVM() + theme(axis.title.x = element_blank(),
                      legend.title = element_blank(), 
                      legend.position = 'none')
...
all_spp_grps <- sort(unique(c(wd_spp2$spp_grp, tr_sum$spp_grp)))
new_dat <- data.frame(spp_grp = all_spp_grps, y = 1:length(all_spp_grps), x = all_spp_grps)

fake_plot <- ggplot(new_dat, aes(x = x, y = y, fill = spp_grp)) + 
...
full_plot <- 
  plot_grid(
  plot_grid(seed_dens_plot, sap_dens_plot, tr_dens_plot, tr_BA_plot, ncol = 4, nrow = 1),
  spp_leg, ncol = 1, nrow = 2, rel_heights = c(4, 2))

full_plot

ggsave(paste0(new_path, "figures/", "Figure_4_", park, "_tree_sapling_regen_by_species.svg"),
       height = 6.5, width = 7.5, units = 'in')

ggsave(paste0(new_path, "figures/", "Figure_4_", park, "_tree_sapling_regen_by_species.png"),
       height = 6.5, width = 7.5, units = 'in', dpi = 600)
