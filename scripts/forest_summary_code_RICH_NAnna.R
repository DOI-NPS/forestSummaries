...
#---- Plot event lists ----
NAnna_plots <- paste0("RICH-", 410:414)
plotevs <- do.call(joinLocEvent, args_all) |> filter(Plot_Name %in% NAnna_plots) |> change_cycle()
plotevs_vs <- do.call(joinLocEvent, args_vs) |> filter(Plot_Name %in% NAnna_plots) |> change_cycle()
plotevs_4yr1 <- plotevs %>% filter(between(SampleYear, from_4yr, to))  |> change_cycle()
head(plotevs)
...
reg <- do.call(joinRegenData, 
               args = c(args_all, speciesType = 'native', 
                        canopyForm = 'canopy', units = 'sq.m')) |> filter(Plot_Name %in% NAnna_plots) |> change_cycle()
...
reg_cycle2 <- reg_cycle3 %>% mutate(MAX = max(across(starts_with("cycle")), na.rm = T))

reg_cycle <- reg_cycle2 %>% mutate(MIN = min(across(starts_with("cycle")), na.rm = T))

#max(reg_cycle[,7:ncol(reg_cycle)], na.rm = T)
...
reg_vs <- do.call(joinRegenData, 
                  args = c(args_vs, speciesType = 'native', 
                           canopyForm = 'canopy', units = 'sq.m')) |> filter(Plot_Name %in% NAnna_plots) |> 
  change_cycle()
...
tree_dd <- do.call(sumTreeDBHDist, args = c(args_vs, status = 'live')) |> 
  filter(Plot_Name %in% NAnna_plots) |> 
  change_cycle()
...
tree_dbh_sum <- suppressWarnings(# warning is for 'c', which we're not using
  ...
  lin_mod <- lm(mean ~ class, data = tree_dbh_sum)
  exp_mod <- lm(log(mean + e) ~ class, data = tree_dbh_sum)
  AIC_test <- data.frame(cycle = 1,
                          linear = AIC(lin_mod),
                          exp = AIC(exp_mod) + sum(2*log(tree_dbh_sum$mean + e)))
...
reg_all <- do.call(joinRegenData, args = c(args_4yr, units = 'sq.m')) |> 
  ...
  filter(Plot_Name %in% NAnna_plots) |> change_cycle()
...
tree_4yr <- do.call(joinTreeData, args = c(args_4yr, status = 'live')) |> filter(Plot_Name %in% NAnna_plots) 
...
trees_park <- do.call(joinTreeData, args = list(park = park, from_4yr, to = to, status = 'live')) |> # just selected park
  change_cycle() |> filter(Plot_Name %in% NAnna_plots) 
...
stand_4yr <- do.call(joinStandData, args_4yr) |> filter(Plot_Name %in% NAnna_plots) |> change_cycle()
...
invcov_wide2 <- invcov_wide3 %>% mutate(MAX = max(across(starts_with("cycle")), na.rm = T))

invcov_wide <- invcov_wide2 %>% mutate(MIN = min(across(starts_with("cycle")), na.rm = T))
...
cancov_wide2 <- cancov_wide3 %>% mutate(MAX = max(across(starts_with("cycle")), na.rm = T))

cancov_wide <- cancov_wide2 %>% mutate(MIN = min(across(starts_with("cycle")), na.rm = T))
...
inv_spp_final$InvasiveMIDN[inv_spp_final$ScientificName == "Poncirus trifoliata"] <- "Yes"
...
cwd_wide2 <- cwd_wide3 %>% mutate(MAX = max(across(starts_with("cycle")), na.rm = T))

cwd_wide <- cwd_wide2 %>% mutate(MIN = min(across(starts_with("cycle")), na.rm = T))
...
frax_wide2 <- frax_wide3 %>% mutate(MAX = max(across(starts_with("cycle")), na.rm = T))

frax_wide <- frax_wide2 %>% mutate(MIN = min(across(starts_with("cycle")), na.rm = T))
