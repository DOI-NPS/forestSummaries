...
params:
  park: 'WEFA'
  report_year: 2026
  make_maps: TRUE # FALSE if not making ArcGIS maps; drops Table 5
  partial_cycle: FALSE
---
...
# 3. Update cycle_labs in source_script_NETN.R
...
partial_cycle <- params$partial_cycle
```
...
} else {source('source_script_NETN.R')} # sets up parameters. 
```
...
} else {source('./scripts/forest_summary_code_NETN.R')}
```
```{r include = F, cache = F}
...
cap_t2 <- paste0("<p style='font-size:10px; padding:0 5px;'><b>Table 2.</b> Average % cover and number of invasive species detected in each plot by cycle. ", paste0(cycle_labs, collapse = "; "), ". Plots are sampled in 4-year rotations, with a quarter of the plots sampled each year (i.e. a panel). ", " Plots with at least 10% average cover or 4 or more invasive species present are in yellow. ", ifelse(partial_cycle == TRUE, "<b>Note that the latest cycle is only partially complete.</b>", ""), 
if(park == "MORR"){"Note that Plot 14 and 25 are in a deer exclosure established in 2007."}, "</p>")

cap_t2_cont <- paste0("<p style='font-size:10px; padding:0 5px;'><b>Table 2 (cont.).</b> Average % cover and number of invasive species detected in each plot by cycle. ", paste0(cycle_labs, collapse = "; "), ". Plots are sampled in 4-year rotations, with a quarter of the plots sampled each year (i.e. a panel). ", " Plots with at least 10% average cover or 4 or more invasive species present are in yellow. ", ifelse(partial_cycle == TRUE, "<b>Note that the latest cycle is only partially complete.</b>", ""),
if(park == "MORR"){"Note that Plot 14 and 25 are in a deer exclosure established in 2007."}, "</p>")

cap_t3 <- if(park %in% "MORR"){
    paste0("<p style='font-size:10px; padding:0 5px;'><b>Table 3.</b> Number of plots out of ", num_plots, " where priority invasive species were detected. ", "Priority invasive species are those that capable of dominating Eastern US forest habitats. Note that Plot 14 and 25 are in a deer exclosure, and Plot 14 was not part of the original sample design. ", paste0(cycle_labs, collapse = "; "), "</p>"), ifelse(partial_cycle == TRUE, "<b>Note that the latest cycle is only partially complete.</b>", ""),  
} else if(park %in% c("MIMA", "ROVA", "SARA", "WEFA")){
    paste0("<p style='font-size:10px; padding:0 5px;'><b>Table 3.</b> Number of plots out of ", num_plots, " where priority invasive species were detected. ", "Priority invasive species are those that capable of dominating Eastern US forest habitats.", paste0(cycle_labs, collapse = "; "), "</p>")
} else {
  paste0("<p style='font-size:10px; padding:0 5px;'><b>Table 3.</b> Number of plots out of ", num_plots, " where exotic species were detected. ", paste0(cycle_labs, collapse = "; "), ".  Priority invasive species capable of dominating Eastern US forest habitats are highlighed in yellow. ", "</p>")
  }
...
if(!park %in% c("ROVA", "SARA", "ACAD")){
  tab1_kab(park)
} else if(park %in% c("ROVA", "SARA")){
  tab1_kab(park, 1, 30, "table1a")
  tab1_kab(park, 31, num_plots, "table1b")
} else if(park %in% "ACAD"){
...
if(!park %in% c("ROVA", "SARA", "ACAD")){
  ...
  tab2_kab("ACAD", 1, 30, "table2a")
  tab2_kab("ACAD", 31, 60, "table2b")
  tab2_kab("ACAD", 61, 90, "table2c")
  tab2_kab("ACAD", 91, 120, "table2d")
  tab2_kab("ACAD", 121, 150, "table2e")
  tab2_kab("ACAD", 151, num_plots, "table2f")
}

```
```{r include = F, eval = T, results = 'hide'}
# Table 3 setup
tab3_cols <- if(park %in% c("MIMA", "MORR", "ROVA", "SARA", "WEFA")){c("Latin Name", "Common Name", 
                                  gsub("cycle_", "Cycle ", names(inv_spp[,4:ncol(inv_spp)])))
  } else {c("Latin Name", "Common Name", "Invasive", 
            gsub("cycle_", "Cycle ", names(inv_spp[,4:ncol(inv_spp)])))}

inv_spp <- if(park %in% c("MIMA", "MORR", "ROVA", "SARA", "WEFA")){
  inv_spp |> filter(!ScientificName %in% "None present") |> 
    select(-InvasiveNETN)
} else {
  inv_spp |> filter(!ScientificName %in% "None present")
}
...
tab3 <- 
  if(park %in% c("MIMA", "MORR", "ROVA", "SARA", "WEFA")){
...
```{r}
include_fig7 <- if(park == "MORR"){TRUE} else {FALSE}
```

```{r out.width = "98%", out.height = "50%", eval = include_fig7}
include_graphics(paste0(new_path, "figures/Figure_7_", park, "_invasive_cover_vs_stocking_index.svg"))
```

```{r results = 'asis', eval = include_fig7}
cat("<p><b>Figure 7.</b> Relationship between invasive shrub cover and regeneration stocking index with loess smoother (blue line) in ", park_long, ". Red dashed line indicates the 10% invasive shrub cover line that regeneration tends to disappear at levels beyond. The green dashed horizontal line indicates the level above which forests are considered sufficiently stocked. Grey dots indicate an individual plot visit.</p>", "\n\n", sep = "")
```
<br>
...
```{r}
tab1_eval <- ifelse(park %in% c("ACAD", "ROVA", "SARA"), FALSE, TRUE)
tab1b_eval <- ifelse(park %in% c("ACAD", "ROVA", "SARA"), TRUE, FALSE)
tab1c_eval <- ifelse(park %in% "ACAD", FALSE, FALSE)
```
...
```{r echo = F, eval = tab1b_eval, warning = F, message = F}
includeHTML("table1a.html")
```

```{r results = 'asis', eval = tab1b_eval, warning = F, message = F}
cat("### {.page-break-before}", "\n\n")
includeHTML("html_header.html")
```

```{r results = 'asis', eval = tab1b_eval}
cat(cap_t1_cont)
```
...
```{r results = 'asis', eval = tab1c_eval, warning = F, message = F}
cat("### {.page-break-before}", "\n\n")
includeHTML("html_header.html")
```

```{r results = 'asis', eval = tab1c_eval}
cat(cap_t1_cont)
```

```{r echo = F, eval = tab1c_eval, warning = F, message = F}
...
```{r}
tab2_eval <- ifelse(park %in% c("ACAD", "ROVA", "SARA"), FALSE, TRUE)
tab2_evalb <- ifelse(park %in% c("ACAD", "ROVA", "SARA"), TRUE, FALSE)
tab2_evalc <- ifelse(park %in% "ACAD", TRUE, FALSE)
```
